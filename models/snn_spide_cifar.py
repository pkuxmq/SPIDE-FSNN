...
class SNNSPIDEConvMultiLayerNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.network_x = SNNConv(self.c_in, self.c_hidden, self.kernel_size_x, bias=True, stride=self.stride_x, padding=self.padding_x, dropout=self.dropout)

        self.network_s1 = SNNConv(self.c_hidden, self.c_s1, self.kernel_size_s, bias=True, stride=1, dropout=self.dropout)
        self.network_s2 = SNNConv(self.c_s1, self.c_s2, self.kernel_size_s, bias=True, stride=2, dropout=self.dropout)
        self.network_s3 = SNNConv(self.c_s2, self.c_s3, self.kernel_size_s, bias=True, stride=1, dropout=self.dropout)
        self.network_s4 = SNNConv(self.c_s3, self.c_s4, self.kernel_size_s, bias=True, stride=1, dropout=self.dropout)

        self.network_s5 = SNNConvTranspose(self.c_s4, self.c_hidden, bias=False, kernel_size=3, stride=2, padding=1, output_padding=1, dropout=self.dropout)

        if self.leaky == None:
            self.snn_func = SNNIFFuncMultiLayer(nn.ModuleList([self.network_s1, self.network_s2, self.network_s3, self.network_s4, self.network_s5]), self.network_x, vth=self.vth, vth_back=self.vth_back, u_rest=self.u_rest, u_rest_back=self.u_rest_back)
        else:
            self.snn_func = SNNLIFFuncMultiLayer(nn.ModuleList([self.network_s1, self.network_s2, self.network_s3, self.network_s4, self.network_s5]), self.network_x, vth=self.vth, leaky=self.leaky, vth_back=self.vth_back, u_rest=self.u_rest, u_rest_back=self.u_rest_back)

        self.network_s5._wnorm(norm_range=2.)
